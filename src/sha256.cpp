...
const std::string sha256_hash(const std::string filename) {

	FILE* file = fopen(filename.c_str(), "r");
	
	if (file == NULL) {
		throw SHA256Error("Cannot open file for reading: " + std::string(strerror(errno)));
	}
	
	char buffer[32];
	sha256_stream(file, buffer);
